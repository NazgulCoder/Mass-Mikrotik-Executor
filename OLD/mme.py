...
# Function to log the result of an operation to a file
...
def ssh_login_execute(ip, users, passwords, ports, command):
        ...
        for password in passwords:
            for port in ports:
                try:
                    ...
                    if error:
                        log_result(command_log_path, f"{ip} after command execution responded with error: {error}")
                    else:
                        log_result(command_log_path, f"{ip} after command execution responded: {output}")
...
# Read the lists of IPs, usernames, passwords, ports, and command from files
...
command = read_file_to_list(os.path.join(config_path, 'command.txt'))[0]

# Iterate over each IP and attempt to execute the command
for ip in mikrotik_ips:
    ssh_login_execute(ip, users, passwords, ports, command)
